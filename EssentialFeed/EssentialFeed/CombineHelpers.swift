...
extension FeedCache {
    func saveIgnoringResult(_ feedImage: [FeedImage]) {
        self.save(feedImage, completion: {_ in})
    }
    
    func saveIgnoringResult(_ page: Paginated<FeedImage>) {
        self.save(page.items, completion: {_ in})
    }
}
...
typealias AnyDispatchQueScheduler = AnyScheduler<DispatchQueue.SchedulerTimeType, DispatchQueue.SchedulerOptions>

extension AnyDispatchQueScheduler {
...
struct AnyScheduler< SchedulerTimeType : Strideable, SchedulerOptions> : Scheduler where SchedulerTimeType.Stride : SchedulerTimeIntervalConvertible
{
    ...
    private let _scheduleAfterCancellable: (SchedulerTimeType, SchedulerTimeType.Stride,  SchedulerTimeType.Stride, SchedulerOptions?, @escaping () -> Void) -> Cancellable
    ...
    init<S>(_ scheduler: S) where SchedulerTimeType == S.SchedulerTimeType, SchedulerOptions == S.SchedulerOptions, S : Scheduler {
        ...
        _scheduleAfterCancellable = scheduler.schedule(after:interval:tolerance:options:_:)
    }
